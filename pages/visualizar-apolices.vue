                    ...
                    <div class="card mb-2 mt-2 mr-auto text-light  shadowCustom" style="background:rgba(0,0,0,0.6)"  :id="i">
                        <div class="card-body">
                            <div class="card-title ">
                                Seguradora: {{arrayApolices[i - 1]['seguradora']['apelido']}}
                            </div>
                            <div class=" card-text ">
                                Numero: {{arrayApolices[i - 1]['numero']}}
                            </div>
                            <div class="card-text ">
                                Data de emissao: {{arrayApolices[i - 1]['data_emissao']}}
                            </div>
                         
                            <div class="card-text ">
                                Data de validade: {{arrayApolices[i - 1]['data_validade']}}
                            </div>
                            ...
                            <div class="row">
                                <div class="col-3 ml-auto mr-3 pl-0 pr-0">
                                    <button type="button" class="btn fs-custom col-12 p-0 btn-outline-success" >
                                        <router-link  :to="{ path: '/cadastrar-apolice', query: { id: arrayApolices[i-1]['id'] }}">Ver Mais</router-link> 
                                    </button>
                                </div>
                                <div class="col-3  pl-0 pr-0">
                                    <button type="button" class="btn fs-custom col-9 p-0 btn-outline-warning">
                                        <router-link  :to="{ path: '/cadastrar-apolice', query: { id: arrayApolices[i-1]['id'], editar:true }}">Editar</router-link> 
                                    </button>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    beforeCreate(){
        ...
        .then(response => {
            ...
            this.tamanhoArray = this.arrayApolices.length
