                    ...
                    <div class="card mb-2 mt-2 mr-auto text-light  shadowCustom" style="background:rgba(0,0,0,0.6)"  :id="i">
                        <div class="card-body">
                            <div class="card-title ">
                                Contratado: {{arrayContratos[i - 1]['contratado']}}
                            </div>
                            <div class=" card-text ">
                                Cargo: {{arrayContratos[i - 1]['cargo']}}
                            </div>       
                            <div class="row">
                                <div class="col-3 ml-auto mr-3 pl-0 pr-0">
                                    <button type="button" class="btn fs-custom col-12 p-0 btn-outline-success" >
                                        <router-link  :to="{ path: '/cadastrar-contrato', query: { id: arrayContratos[i-1]['id'] }}">Ver Mais</router-link> 
                                    </button>
                                </div>
                                <div class="col-3  pl-0 pr-0">
                                    <button type="button" class="btn fs-custom col-9 p-0 btn-outline-warning">
                                        <router-link  :to="{ path: '/cadastrar-contrato', query: { id: arrayContratos[i-1]['id'], editar:true }}">Editar</router-link> 
                                    </button>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    beforeCreate(){
        ...
        axios
        .get(`${process.env.apiURL}contratos`,{headers})
        .then(response => {
            this.arrayContratos = response.data
            this.tamanhoArray = this.arrayContratos.length
